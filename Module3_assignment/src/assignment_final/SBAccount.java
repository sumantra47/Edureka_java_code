...
public final class SBAccount extends Account {
	...
	static final float MIN_BALANCE = 3000;
	static final int INTEREST = 4;

	public SBAccount(String account_name, String account_number, float amount) {
	...
	public void checkbalance(String account_number, int month) {
		if (this.account_number.equals(account_number)) {
			this.amount = this.amount + (this.amount * INTEREST * month) / 100;
			System.out.println("Account Number :" + account_number);
			System.out.println("Balance : " + this.amount);
		} else {
			System.out.println("Invalid Account number: " + account_number);
